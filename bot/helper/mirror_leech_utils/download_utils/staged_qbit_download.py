...
class StagedQbitCoordinator:
    ...
    async def load_manifest(self):
        ...
        max_upload_size = (
            self.listener.max_split_size if self.listener.is_leech else 0
        )
    ...
    async def _delete_batch(self):
        ...
        await clean_download(self._stage_dir)
        if failures:
            raise RuntimeError("Could not delete uploaded files: " + "; ".join(failures))
    ...
    async def run(self):
        try:
            ...
            while self.pending and not self.listener.is_cancelled:
                ...
                self.pending = [item for item in self.pending if item.index not in batch_ids]
                self.current_batch = []
...
async def add_staged_qb_torrent(listener, path):
    ...
    while True:
        torrent = (await TorrentManager.qbittorrent.torrents.info(hashes=[torrent.hash]))[0]
